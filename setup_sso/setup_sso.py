...
class AccountSetup:
    ...
    start_of_policy = (
        '{"Version":"2012-10-17","Statement":[{"Action":"sts:AssumeRoleWithSAML","Effect":"Allow",'
    ...
    def __init__(
        ...
        self.account_id = (
            self.session.client("sts", endpoint_url=self.endpoint_url).get_caller_identity().get("Account")
        )
        self.aws_partition = self.session.client("ec2", endpoint_url=self.endpoint_url).meta.partition
        self.saml_provider_arn = (
            f"arn:{self.aws_partition}:iam::{self.account_id}:saml-provider/{self.saml_provider_name}"
        )
        self.roles_arn = {
    ...
    def alias(self) -> None:
        ...
        if self._check_alias(self.alias_name):
            ...
            try:
                self.client.create_account_alias(AccountAlias=self.alias_name)
    ...
    def saml(self) -> None:
        ...
        if self._check_saml_provider():
            ...
            with self.saml_provider_file.open() as f:
                try:
                    saml = self.client.create_saml_provider(SAMLMetadataDocument=f.read(), Name=self.saml_provider_name)
    ...
    def _check_saml_provider(self) -> bool:
        ...
        saml_providers = [provider["Arn"] for provider in self.client.list_saml_providers()["SAMLProviderList"]]
    ...
    def _check_role(self, role_name: str) -> bool:
        current_roles = [role["Arn"] for role in self.client.list_roles()["Roles"]]
    ...
    def create_default_roles(self) -> None:
        ...
        try:
            ...
            self.create_role(role_name=admin_role_name, policies=admin_policies, policy_document=policy_document)
            read_role_name = "SSOViewOnlyAccess"
            read_policies = [f"arn:{aws_partition}:iam::aws:policy/ReadOnlyAccess"]
            self.create_role(role_name=read_role_name, policies=read_policies, policy_document=policy_document)
            dev_role_name = "SSODeveloperAccess"
            dev_policies = [f"arn:{aws_partition}:iam::aws:policy/job-function/SystemAdministrator"]
            self.create_role(role_name=dev_role_name, policies=dev_policies, policy_document=policy_document)
        except Exception as e:
